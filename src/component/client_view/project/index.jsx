            ...
            ? data.map((projectItem, index) => (
                ...
                >
                  <div className="border-2 w-full relative border-green-main transition-all rounded-lg flex flex-col">
                      ...
                      <div className="flex order-2 xl:order-1">
                        <div className="flex flex-col">
                          ...
                          <p className="text-sm mt-2 text-black-500 capitalize font-bold">
                            {projectItem.createdAt.split("T")[0]}
                          </p>
                          ...
                          <div className="grid gap-2 mt-5 grid-cols-2 h-full max-h-[200px] w-full">
                            {projectItem?.technologies
                              ...
                              .map((techItem, index) => (
                                ...
                                >
                                  <button
                                    className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[120px] px-6
                                  border-[2px] border-green-main bg-[#fff] text-[#000] font-semibold rounded-lg
                                  text-xs -tracking-widest hover:shadow-green-main transition-all outline-none"
                                  >
                                    {techItem}
                                  </button>
                                </div>
                    ...
                    <div className="absolute w-full bottom-0 justify-center flex gap-2 pt-3">
                      <button
                        onClick={() => router.push(projectItem.website)}
                        className="p-2 cursor-pointer text-white-300 font-semibold text-[14px] tracking-widest bg-[#1E3A5F] transition-all outline-none rounded"
                      >
                        Website
                      </button>

                      <button
                        onClick={() => router.push(projectItem.github)}
                        className="p-2 cursor-pointer text-white-300 font-semibold text-[14px] tracking-widest bg-[#1E3A5F] transition-all outline-none rounded"
                      >
                        Github
                      </button>
